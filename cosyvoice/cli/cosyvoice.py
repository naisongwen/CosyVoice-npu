...
def _is_accel_available():
    """Return True on CUDA or Ascend NPU."""
...
class CosyVoice:
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, fp16=False, trt_concurrent=1):
        ...
        if not _is_accel_available() and (load_jit is True or load_trt is True or fp16 is True):
            load_jit, load_trt, fp16 = False, False, False
            logging.warning('no accelerator device, set load_jit/load_trt/fp16 to False')
...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        self.frontend = CosyVoiceFrontEnd(configs['get_tokenizer'],
                                          ...
                                          configs['allowed_special'])
        self.sample_rate = configs['sample_rate']
        if not _is_accel_available() and (load_jit is True or load_trt is True or load_vllm is True or fp16 is True):
            load_jit, load_trt, load_vllm, fp16 = False, False, False, False
            logging.warning('no accelerator device, set load_jit/load_trt/load_vllm/fp16 to False')
...
class CosyVoice3(CosyVoice2):
    ...
    def __init__(self, model_dir, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        self.frontend = CosyVoiceFrontEnd(configs['get_tokenizer'],
                                          ...
                                          configs['allowed_special'])
        self.sample_rate = configs['sample_rate']
        if not _is_accel_available() and (load_trt is True or fp16 is True):
            load_trt, fp16 = False, False
            logging.warning('no accelerator device, set load_trt/fp16 to False')
